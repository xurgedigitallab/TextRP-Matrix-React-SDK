...
class AutoDiscovery {
  ...
  static async validateDiscoveryAuthenticationConfig(wellKnown) {
    try {
      const homeserverAuthenticationConfig = (0, _validate.validateWellKnownAuthentication)(wellKnown);
      ...
      if (issuerWellKnown.action !== AutoDiscoveryAction.SUCCESS) {
        ...
        throw new Error(_validate.OidcDiscoveryError.General);
      }
      ...
      const errorType = Object.values(_validate.OidcDiscoveryError).includes(errorMessage) ? errorMessage : _validate.OidcDiscoveryError.General;
      const state = errorType === _validate.OidcDiscoveryError.NotSupported ? AutoDiscoveryAction.IGNORE : AutoDiscoveryAction.FAIL_ERROR;
      return {
        state,
        error: errorType
      };
